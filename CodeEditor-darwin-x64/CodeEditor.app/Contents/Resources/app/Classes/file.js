...
class File {
    ...
    createFile() {
        ...
        fs.writeFile(__dirname + '/Files/'+ this.fileName +'.json', code, (err) => {console.log(err)})
      ...
      fs.writeFile(__dirname + '/Files/'+ this.fileName +'.html', newPage, (err) => {
        if (err) {
          return
        }
      })
    ...
    showFile(filename){
        ...
        let tab = tabGroup.addTab({
          title: filename,
          src: __dirname + '/Files/' + filename + '.html',
          webviewAttributes: {
    ...
    saveFile(){
        var jsonFile = fs.readFileSync(__dirname + '/Files/' + this.fileName + '.json', 'utf8', (err)=> {
            if (err) {
              return
             };
          });
          ...
          fs.writeFile(__dirname + '/Files/' + this.fileName + '.json', jsonFileContent, (err) => {
            if (err){return}
          })
        }
